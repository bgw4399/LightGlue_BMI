...
def plot_images(imgs, titles=None, cmaps="gray", dpi=100, pad=0.5, adaptive=True):
    ...
    if not isinstance(cmaps, (list, tuple)):
        cmaps = [cmaps] * n
    # if adaptive:
    #     heights = [i.shape[0] / i.shape[1] for i in imgs]  # H / W
    # else:
    #     heights = [3 / 4] * n
    # figsize = [4.5, sum(heights) * 4.5]  # Adjust figsize for vertical stacking
    # fig, ax = plt.subplots(
    #     n, 1, figsize=figsize, dpi=dpi, gridspec_kw={"height_ratios": heights}
    # )
    if adaptive:
...
def plot_keypoints(kpts, colors="lime", ps=2, axes=None, a=1.0):
